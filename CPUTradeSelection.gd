...
signal cpu_selection_made(selection)

func _ready():
	team_roster = Rosters.team_rosters[selected_team]
...
func _on_select_pressed():
	...
	if selected_player != "":
		...
		pass
	emit_signal("cpu_selection_made", selection)
...
func get_pick_values():
	...
	for pick in draft_array:
		...
		if pick["year"] == lowest_year:
			...
			year_mult = .7
		pick["Trade Value"] = value * year_mult
