...
var team_roster = Rosters.team_rosters["Free Agent"]
...
func _on_sign_pressed():
	...
	var players_at_position = selected_team_roster.filter(func(p): return p["Position"] == player_to_sign["Position"])
	players_at_position.sort_custom(func(a, b): return a["DepthChart"] < b["DepthChart"])
	...
	for i in range(players_at_position.size()):
		if player_to_sign["Rating"] > players_at_position[i]["Rating"]:
			...
			break
	if not inserted:
		var next_depth_chart = players_at_position[-1]["DepthChart"] + 1 if players_at_position.size() > 0 else 1
		player_to_sign["DepthChart"] = next_depth_chart
	...
	selected_team_roster.append(player_to_sign)
	roster_array.erase(player_to_sign)
	clear_labels()
	populate_fa_list("FAScroll/FreeAgents")

func _on_back_pressed():
	Rosters.team_rosters["Free Agent"] = roster_array
	Rosters.team_rosters[selected_team] = selected_team_roster
